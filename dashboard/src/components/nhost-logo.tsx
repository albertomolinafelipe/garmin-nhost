...
export function NhostLogo({ className }: { className?: string }) {
	const id = useId();
	const triangle = `${id}-triangle`;
	const ribbons = `${id}-ribbons`;
	const cut = `${id}-cut`;
	...
	return (
		...
		>
			<defs>
				<path
					id={triangle}
					d="M14.4-3a2 2 0 0 1 3.2 0l22.4 38a2 2 0 0 1-1.7 3H-6.3a2 2 0 0 1-1.7-3L14.4-3Z"
				/>
				<path
					id={ribbons}
					transform="translate(16 22.9) scale(0.66) translate(-16 -17)"
				...
				<mask id={cut}>
					...
					<use href={`#${ribbons}`} fill="black" />
				</mask>
			</defs>
			<use href={`#${triangle}`} fill="#007cc2" mask={`url(#${cut})`} />
		</svg>
	);
}
